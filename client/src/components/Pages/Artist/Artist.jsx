...
cursor: pointer;
`
const Artist = ({ AllArtist, isMobile, color, handleData, handleDownload, selectedSong, handlePlayAll, isPlaying }) => {
  const [total, setTotal] = useState(null);
  const navigate = useNavigate();

  const { id } = useParams();
  const filterArtist = AllArtist.filter(artist => artist.id === id);

  useEffect(() => {
    if (filterArtist.length > 0) {
      setTotal(filterArtist[0].playlist.length);
    }
  }, [filterArtist]);
            ...
            <Wrap key={artist.id}>
              ...
              <Box>
                <BannerWrapper>
                  ...
                  <Box>
                    {isPlaying ? <Pause /> : <PlayAll onClick={() => handlePlayAll(artist.playlist)} />}
                  </Box>
                </BannerWrapper>
              </Box>
              <ul>
                ...
                {artist.playlist.map((song, index) => (
                  <li key={song.id}>
                    <PlayList>
                      ...
                      <Box>
                        <FileDownloadOutlinedIcon onClick={() => handleDownload(song.file, song.title)} style={{ padding: "0 5", fontSize: 20 }} />
                      </Box>
                    </PlayList>
                  </li>
                ))}
                  ...
                  <TableBody>
                    {artist.playlist.map((song, index) => (
                      <TableRow key={song.id} align='center' md={{ '&:last-child td, &:last-child th': { border: 0 } }} >
                        ...
                        <TableCell>
                          <ImageWrapper
                            onClick={() => handleData(song, index, artist.playlist)}>
                            <img src={song.url} alt={song.title} style={{ width: 50 }} />
                            <Typography variant='p' style={{ color: selectedSong === song ? '#3FB5E2' : 'inherit' }}
