...
cursor: pointer;
`
const RomanticPlaylist = ({romantic_songs,isMobile, color,handleData,handleDownload,currentSongColor,selectedSong,handlePlayAll,isPlaying}) => {
 const [total,setTotal]=useState(null);
 const navigate = useNavigate();
  const { id } = useParams();
  const Filteralbum = romantic_songs.filter(album => album.id === id);
        ...
        <Component>
          {Filteralbum.map(album => (
            <Wrap key={album.id}>
              ...
              <Box>
                <BannerWrapper>
                  <ArtistName variant="h5">{album.title}</ArtistName>
                  <Box>
                  ...
                  <TableBody>
                    {album.playlist.map((song, index) => (
                      <TableRow key={song.id} align='center'md={{ '&:last-child td, &:last-child th': { border: 0 } }} >
                        ...
                        <TableCell>
                          <ImageWrapper onClick={()=>handleData(song,index,album.playlist)} >
